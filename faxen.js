...
function getPreviousLink(from, to) {
  ...
  return "<a class=\"previous\" href=\"" + getUrlFromPages(fromPrevious, toPrevious) + "\" style=\"" + stylePrevious +
    "\">&lt;- Föregående</a>";
}
...
function getNextLink(from, to) {
  ...
  return "<a class=\"next\" href=\"" + getUrlFromPages(fromNext, toNext) + "\" style=\"" + styleNext + "\">Vidare -></a>";
}
...
function getNumberedLinks(currentFrom) {
  ...
  for (var from = 1; from <= NO_PAGES; from += PAGES_PER_PAGE) {
    ...
    if (from === currentFrom) {
      ...
      linksHtml += "<a href=\"" + getUrlFromPages(from, to) + "\">" + fromTo + "</a>";
    }
  }
  linksHtml += "</div>";
  return linksHtml;
